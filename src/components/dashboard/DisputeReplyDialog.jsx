    ...
    try {
      ...
      for (const admin of admins) {
        await base44.entities.Notification.create({
          user_id: admin.id,
          title: `⚖️ Réponse au Litige ${dispute.dispute_code}`,
        ...
        // Email : une réponse a été soumise, le dossier nécessite un nouvel examen
        if (admin.email) {
          await SendEmail({
            to: admin.email,
            subject: `⚖️ Réponse au litige ${dispute.dispute_code}`,
            body: `La partie adverse (${party}) a soumis son contre-argument sur le litige ${dispute.dispute_code}.\n\nLe dossier est à revoir : ${window.location.origin}/AdminDashboard?tab=disputes\n\nCordialement,\nL'équipe EventCrafter`
          });
        }
      }
