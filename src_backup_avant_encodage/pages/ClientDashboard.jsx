...
export default function ClientDashboard() {
  ...
  const createEventMutation = useMutation({
    mutationFn: async ({ eventName, eventDate, eventDescription, cart }) => {
      ...
      console.log("�?vénement créé:", event);
      ...
      for (const cartService of cart) {
        ...
        await base44.entities.Notification.create({
          user_id: vendorId,
          title: "�Y"< Nouvelle demande de réservation",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['events', 'bookings']);
      toast({ title: "�?vénement créé!", description: `Services ajoutés avec succès` });
    },
    onError: (error) => {
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsList className="w-max min-w-full justify-start bg-stone-100 p-1">
          <TabsTrigger value="contact_cart" className="px-3 sm:px-6 text-xs sm:text-sm whitespace-nowrap">�Y>' Panier</TabsTrigger>
          <TabsTrigger value="my_bookings" className="px-3 sm:px-6 text-xs sm:text-sm whitespace-nowrap">�Y". �?vénements</TabsTrigger>
          <TabsTrigger value="my_requests" className="px-3 sm:px-6 text-xs sm:text-sm whitespace-nowrap">�Y"� Demandes</TabsTrigger>
          <TabsTrigger value="client_profile" className="px-3 sm:px-6 text-xs sm:text-sm whitespace-nowrap">�Y'� Profil</TabsTrigger>
              ...
              {cart.length > 0 ? (
                <div className="space-y-6">
                      ...
                      <div key={service.id} className="flex items-start gap-4 p-4 border rounded-lg bg-stone-50 hover:bg-stone-100 transition-colors group">
                        ...
                        <div className="flex-grow">
                          ...
                          <p className="text-sm font-bold text-rose-600 mt-1">
                            �? partir de {service.price_min?.toLocaleString()} FCFA
                          </p>
                        </div>
                  ...
                  <div className="border-t pt-6 space-y-4">
                    <h3 className="font-semibold text-lg">Créer un �?vénement</h3>
                    ...
                    >
                      {createEventMutation.isPending ? (
                        ...
                        <>
                          ...
                          Créer �?vénement avec {cart.length} Service{cart.length > 1 ? 's' : ''}
                        </>
                      )}
                    </Button>
                    ...
                    <Card key={lead.id} className="border-l-4 border-rose-600">
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <p className="text-stone-600">�Y"� {lead.location}</p>
                          {lead.budget && <p className="text-stone-600">�Y'� {lead.budget}</p>}
